...
public class GraphFoundationDAO implements AutoCloseable {
	 ...
	 * "match(c:Class {name:'BatchServiceImpl.java'}) return c.name"; try (
	 * Session session = driver.session() ) { String res =
	 * session.writeTransaction( new TransactionWork<String>() {
	 * 
	 * @Override public String execute( Transaction tx ){ StatementResult result
	 * = tx.run(query); System.out.println(result.single().get(0).toString());
	 * return result.single().get(0).toString(); } } ); System.out.println(res);
	 * return true; } }
	 */
	...
	public boolean persistFullClassNode(JavaImplementation impl) throws Exception {
		...
		String attributes = "path:'" + impl.getPath() + "', moduleDeclaration:'" + impl.getModuleDeclaration()
				...
				+ impl.getImplementedInterfaces() + ", extensions:" + impl.getExtensions().toString() + ", fields:"
	...
	public boolean persistEntity(String className, String entityName) {
		...
		query = "MATCH (entityClass {name:'" + className + "'}) MERGE (entityClass)-[:" + relationType
	...
	public boolean persistDependencyInjection(String dependentClass, String injectedClass) {
		...
		if (result.hasNext()) {
			...
			// JavaImplementation)
			query = "MATCH (dependentClass:" + genericNodeType + " {name:'" + dependentClass
	...
	public boolean persistSecondLevelFunctionality(SecondLevelFunctionality secondLevelFunc) {
		...
		query = query + addFirstLevelQuery + addLayerQuery;
		result = session.run(query);
		if (result.summary() != null) {
			return true;
		}
	...
	public boolean associateJavaImplWithFunctionality(String javaImplementationPath, String functionality,
		...
		query = "MATCH (j:JavaImplementation) WHERE j.path='" + javaImplementationPath
	...
	public boolean persistRessource(String resource, String javaImplementationPath) {
		...
		query = "MATCH (j:JavaImplementation) WHERE j.path='" + javaImplementationPath + "' MERGE (r:" + nodeType
	...
	public boolean persistLibrary(Library lib, String javaImplementationPath) {
		...
		query = "MATCH (j:JavaImplementation) WHERE j.path='" + javaImplementationPath + "' MERGE (l:" + nodeType
